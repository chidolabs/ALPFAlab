...
create table if not exists room_sessions (
  ...
  support_volunteer_id uuid references volunteers(id) on delete set null,
  support_volunteer_name text,
  created_at timestamptz not null default now()
);

-- Table already existed before these columns were added; this is a no-op
-- if they're already there (needed since the table above uses "create
-- table if not exists", which won't alter an existing table).
...
alter table room_sessions add column if not exists support_volunteer_id uuid references volunteers(id) on delete set null;
alter table room_sessions add column if not exists support_volunteer_name text;
